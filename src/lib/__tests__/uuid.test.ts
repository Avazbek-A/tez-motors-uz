...
describe("PUBLIC_CAR_LIST_COLUMNS", () => {
  it("drops spec_data (the jsonb that timed out the list query) and nothing else", () => {
    expect(PUBLIC_CAR_LIST_COLUMNS).not.toContain("spec_data");
    expect(PUBLIC_CAR_LIST_COLUMNS.split(", ")).toEqual(
      PUBLIC_CAR_COLUMNS.split(", ").filter((c) => c !== "spec_data"),
    );
  });
