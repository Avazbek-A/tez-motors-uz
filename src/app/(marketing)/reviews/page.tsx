    ...
    try {
      await fetch("/api/reviews", {
        ...
        body: JSON.stringify({
          ...
          car_description: formData.get("car"),
          review_text_ru: formData.get("review"),
          rating,
        }),
      });
      setIsSuccess(true);
      ...
      <div className="container-custom">
        ...
        <div className="flex items-center justify-center gap-8 mb-12">
          <div className="text-center">
            <p className="text-4xl font-bold text-navy">4.9</p>
            <div className="flex gap-0.5 justify-center mt-1">
              {Array.from({ length: 5 }).map((_, i) => (
                <Star key={i} className="w-4 h-4 fill-yellow-400 text-yellow-400" />
              ))}
            </div>
            <p className="text-xs text-muted-foreground mt-1">{reviews.length} {locale === "ru" ? "отзывов" : "reviews"}</p>
          </div>
        ...
        {showForm && (
          <div className="max-w-xl mx-auto mb-12 animate-fade-in-up">
            {isSuccess ? (
              <div className="bg-lime/10 rounded-2xl border border-lime/20 p-8 text-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {reviews.map((review, index) => (
            <div
              key={review.id}
              className="bg-white rounded-2xl border border-border p-6 shadow-sm hover:shadow-lg transition-all duration-300 animate-fade-in-up"
