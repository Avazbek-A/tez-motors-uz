...
export async function GET(request: NextRequest, ctx: { params: Promise<{ slug: string }> }) {
  ...
  let query = supabase.from("scooters").select("*").eq("slug", slug).maybeSingle();
  if (!all) query = supabase.from("scooters").select("*").eq("slug", slug).eq("is_published", true).maybeSingle();

  const { data, error } = await query;
  if (error) {
    reportServerError("GET /api/scooters/[slug]", error).catch(() => {});
