...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const [carsRes, costsRes, paymentsRes, poRes, fx] = await Promise.all([
      // All aggregates paginated so inventory value / margin / deposits don't
      // undercount past the row cap.
      fetchAllRows<{ id: string; price_usd: number; inventory_status: string }>((from, to) =>
        supabase.from("cars").select("id, price_usd, inventory_status").range(from, to)).then((data) => ({ data })),
      ...
      fetchAllRows<{ amount_tiyin: number }>((from, to) =>
        supabase.from("payments").select("amount_tiyin").eq("state", 2).range(from, to)).then((data) => ({ data })),
      fetchAllRows<{ status: string; qty: number; unit_cost_usd: number }>((from, to) =>
        supabase.from("purchase_orders").select("status, qty, unit_cost_usd").range(from, to)).then((data) => ({ data })),
      getFxRates(supabase),
    ]);
